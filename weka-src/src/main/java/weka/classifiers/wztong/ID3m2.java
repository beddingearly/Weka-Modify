 ...
 */
public class ID3m2 extends Classifier {

    /** The node's successors.
     * 保存决策树节点的数组 */
    private ID3m2[] m_Successors;
    ...
    private int positiveClassValue;
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        //Build jlx_ID3 tree
        positiveClassValue = (int)data.instance(0).classValue();
    ...
    private void makeTree(Instances data) throws Exception {
        ...
        for (int i = 0; i < data.numAttributes(); i++){
            ...
            impurityReduce=computeConditionalProb(data,data.attribute(i));
        ...
        if(Utils.eq(maxValue, 0)){
            ...
            m_Successors = new ID3m2[m_Attribute.numValues()];
            for (int j = 0; j < m_Attribute.numValues(); j++) {
                m_Successors[j] = new ID3m2();
    ...
    private double computeConditionalProb(Instances data, Attribute att) throws Exception {
        double conditionalProb = 0.0;
        Instances[] splitData = splitData(data, att);
        double [] PAB = new double[att.numValues()];
        double [] PB = new double[att.numValues()];
        int baseLine = data.numInstances();
        for (int i = 0; i < att.numValues(); i++){ //遍历该属性的所有属性值
            PAB[i] = 0;
            PB[i] = splitData[i].numInstances(); //属性值为i的实例个数
            for(int j = 0; j < splitData[i].numInstances(); j++){
                if(splitData[i].instance(j).classValue() == positiveClassValue){
                    PAB[i] += 1.0; //属性值为i，且classValue为正的个数
                }
            }
        }
        for (int j = 0; j < att.numValues(); j++){
            PB[j] = PB[j] / (double)baseLine;
            PAB[j] = PAB[j] / (double)baseLine;
            conditionalProb += PAB[j] / PB[j];
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            System.out.println(Evaluation.evaluateModel(new ID3m2(), args));
        } catch (Exception e) {
