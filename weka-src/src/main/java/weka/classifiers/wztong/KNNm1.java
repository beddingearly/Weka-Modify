 ...
 */
public class KNNm1 extends Classifier {
   ...
   */
   private NeighborList findNeighbors(Instance instance,int kNN) throws Exception {

    double distance;
    NeighborList neighborlist = new NeighborList(kNN);
    for(int i=0; i<m_Train.numInstances();i++){// 遍历每一个实例
      Instance trainInstance=m_Train.instance(i);// 每一个实例
      distance=distanceBasedOnEntropy(instance,trainInstance);
  ...
  private double distance(Instance first, Instance second) {
    ...
    double distance = 0;
    for(int i=0;i<m_Train.numAttributes();i++){// 遍历实例的每一条属性
      if(i == m_Train.classIndex()) continue;
      if((int)first.value(i)!=(int)second.value(i)){ // double value(int attIndex)
        distance+=1;
      }
    }
    return distance;
  ...
  private double distanceBasedOnEntropy(Instance first, Instance second) throws Exception{
    double distance  = 0.0;
    int cot = 0;
    for(int i = 0; i < m_Train.numAttributes(); i++){
      ...
      if((int)first.value(i) == (int)second.value(i)){ // (double) m_AttValues[attIndex];
        cot += 1;
        distance += computeEntropyReduce(m_Train, first.attribute(i), i, (int)first.value(i));// return m_AttValues[attIndex];
      }
    }
    if (cot == 0){
      return 100;
  ...
  // index 属性位置 value 属性值
  private double computeEntropyReduce(Instances data, Attribute att, int index, int value) throws Exception {
    Instances[] splitData = splitData(data, att);
    return computeEntropy(splitData[value]);
  }
  ...
  private Instances[] splitData(Instances data, Attribute att) {
    ...
    for (int i=0;i<numInstances;i++){
      //
      int attVal=(int)data.instance(i).value(att); //(double) return m_AttValues[attIndex];
  ...
  public static void main(String[] args) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new KNNm1(), args));
    } catch (Exception e) {
