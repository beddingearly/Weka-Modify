...
/**
 * Implement ID3m1 classifier.
 */
public class ID3m3 extends Classifier {

  /** The node's successors. */
  private ID3m3[] m_Successors;
  ...
  private static double numStopNodeSplit = 2;

  /**
   * Builds ID3m1 decision tree classifier.
   *
  ...
  /**
   * Method building ID3m1 tree using information gain measure
   *
   * @param data the training data
  ...
  private void makeTree(Instances data) throws Exception {
    ...
    if (data.numInstances() <= numStopNodeSplit) {
      m_Attribute = null;
   ...
   // Make leaf if information gain is zero, otherwise create successors.
    if(Utils.eq(maxValue, 0)){
      ...
      m_Successors = new ID3m3[m_Attribute.numValues()];
      for (int j = 0; j < m_Attribute.numValues(); j++) {
        m_Successors[j] = new ID3m3();
  ...
  public static void main(String[] args) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new ID3m3(), args));
    } catch (Exception e) {
