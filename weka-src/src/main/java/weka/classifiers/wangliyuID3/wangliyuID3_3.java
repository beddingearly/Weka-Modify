 ...
 */
public class wangliyuID3_3 extends Classifier {

  /** The node's successors. */
  private wangliyuID3_3[] m_Successors;
  ...
  //** ��ֹ���� */
  private double m_StopInfoGain;
  private int m_StopNodeNum;
  ...
  public void buildClassifier(Instances data) throws Exception {
	  ...
	  m_StopInfoGain = 0.1;
	  m_StopNodeNum = data.numClasses() - (int)(0.2*data.numClasses());
    //Build ID3 tree
    makeTree(data);
  }
  ...
  private void makeTree(Instances data) throws Exception {
    ...
    if (data.numInstances() == m_StopNodeNum) {
      m_Attribute = null;
    ...
    //informationGain��ֵ����m_stop��ֹ
    if(maxValue < m_StopInfoGain)
    {
    	 m_Attribute = null;
         m_Instances=new Instances(data);
         return;
   ...
   // Make leaf if information gain is zero, otherwise create successors.
    if(Utils.eq(maxValue, 0)){
      ...
      m_Successors = new wangliyuID3_3[m_Attribute.numValues()];
      for (int j = 0; j < m_Attribute.numValues(); j++) {
        m_Successors[j] = new wangliyuID3_3();
  ...
  public static void main(String[] args) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new wangliyuID3_3(), args));
    } catch (Exception e) {
