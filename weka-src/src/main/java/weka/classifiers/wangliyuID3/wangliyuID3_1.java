 ...
 */
public class wangliyuID3_1 extends Classifier {

  /** The node's successors. */
  private wangliyuID3_1[] m_Successors;
  ...
  private void makeTree(Instances data) throws Exception {
   ...
   // Make leaf if information gain is zero, otherwise create successors.
    if(Utils.eq(maxValue, 0)){
      ...
      m_Successors = new wangliyuID3_1[m_Attribute.numValues()];
      for (int j = 0; j < m_Attribute.numValues(); j++) {
        m_Successors[j] = new wangliyuID3_1();
  ...
  private double computeEntropyReduce(Instances data, Attribute att) throws Exception {
    ...
    //??????????
    entropyReduce = entropyReduce / data.numClasses();
  ...
  public static void main(String[] args) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new wangliyuID3_1(), args));
    } catch (Exception e) {
