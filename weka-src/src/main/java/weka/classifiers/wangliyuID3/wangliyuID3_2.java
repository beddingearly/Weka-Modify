...
package weka.classifiers.wangliyuID3;
 ...
 */
public class wangliyuID3_2 extends Classifier {

  /** The node's successors. */
  private wangliyuID3_2[] m_Successors;
  ...
  private void makeTree(Instances data) throws Exception {
    ...
    //??????
    double [] impuirityReduceArray = new double [data.numAttributes()];
    //?????
    double sum = 0;
    //???????
    double [] impuirityReducePercentagetArray = new double [data.numAttributes()];
    ...
    for (int i = 0; i < data.numAttributes(); i++){
      ...
      impuirityReduceArray[i] = impurityReduce;
      sum += impurityReduce;
    }
    //????????
    for(int i = 0; i < data.numAttributes(); i++)
    {
    	impuirityReduceArray[i] /= sum;
    	if(i == 0)
    		impuirityReducePercentagetArray[i] = impuirityReduceArray[i];
    	else
    		impuirityReducePercentagetArray[i] = impuirityReducePercentagetArray[i-1] + impuirityReduceArray[i];
    }
    //?????
    Random random = new Random();
    double r = (random.nextInt() % 1000)/1000; 
    for(int i = 0; i < data.numAttributes(); i++)
    {
    	if(r <= impuirityReducePercentagetArray[i])
    	{
    		maxValue=impuirityReduceArray[i];
            maxIndex=i;
            break;
    	}
    }
   // Make leaf if information gain is zero, otherwise create successors.
    if(Utils.eq(maxValue, 0)){
      ...
      m_Successors = new wangliyuID3_2[m_Attribute.numValues()];
      for (int j = 0; j < m_Attribute.numValues(); j++) {
        m_Successors[j] = new wangliyuID3_2();
  ...
  public static void main(String[] args) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new wangliyuID3_2(), args));
    } catch (Exception e) {
