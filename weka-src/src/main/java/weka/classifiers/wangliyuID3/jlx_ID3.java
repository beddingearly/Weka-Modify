 ...
 */
public class jlx_ID3 extends Classifier {

  /** The node's successors.
   * 保存决策树节点的数组 */
  private jlx_ID3[] m_Successors;
  ...
  private void makeTree(Instances data) throws Exception {
   ...
   // Make leaf if information gain is zero, otherwise create successors.
    if(Utils.eq(maxValue, 0)){
      ...
      m_Successors = new jlx_ID3[m_Attribute.numValues()];
      for (int j = 0; j < m_Attribute.numValues(); j++) {
        m_Successors[j] = new jlx_ID3();
  ...
  public static void main(String[] args) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new jlx_ID3(), args));
    } catch (Exception e) {
